...
struct Indices {
    ...
    int column = -1;
    int inner = -1;
	Indices(int lx1, int ly1) : lx1(lx1), ly1(ly1) {}
	Indices(int lx1, int ly1, int lx2, int ly2, int row, int column, int inner) : lx1(lx1), ly1(ly1), lx2(lx2), ly2(ly2), row(row), column(column), inner(inner) {}
};
...
struct Pthread {
    ...
    HANDLE mutex;
    Pthread(const Matrix& matrix1, const Matrix& matrix2, const Indices& ind, Matrix& result_main, const Indices& cur_sum, HANDLE mutex) 
		: matrix1(matrix1), matrix2(matrix2), ind(ind), result_main(result_main), cur_sum(cur_sum), mutex(mutex) {
	}
};
...
void deleteVM(std::vector<HANDLE>& th, std::vector<HANDLE> m) {
    int TotalCount = th.size();
    int waitedCount = 0;

    while (waitedCount < TotalCount) {
		int curCount = (TotalCount - waitedCount > MAXIMUM_WAIT_OBJECTS) ? MAXIMUM_WAIT_OBJECTS : TotalCount - waitedCount;
...
DWORD WINAPI matrix_block_multiplication(void* arg) {
	...
	WaitForSingleObject(pthread->mutex, INFINITE);
    matrix_block_addition(pthread->result_main, result, pthread->cur_sum);
    ReleaseMutex(pthread->mutex);
    delete pthread;
...
Matrix matrix_multiplication(const Matrix& matrix1, const Matrix& matrix2, int N, int k, int& count) {
    ...
    int ind_end = (k * (N / k)) == 0 ? -1 : (k * (N / k)), edge_case = N - ind_end;
    for (int i = 0; i < N; i += k) {
        for (int cur = 0; cur < N; cur += k) {
            ...
            for (int j = 0; j < N; j += k) {
                ...
                HANDLE hthread = CreateThread(NULL, 0, matrix_block_multiplication, p.back(), 0, NULL);
                if (hthread) {
					th.push_back(hthread);
                }
            }
        }
    }
